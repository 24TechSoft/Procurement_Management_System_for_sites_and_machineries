...
public partial class Worker : System.Web.UI.MasterPage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (Convert.ToInt32(Request.Cookies["UserType"].Value) != 2)
            {
                if (Convert.ToInt32(Request.Cookies["UserType"].Value) == 3)
                {
                    Response.Redirect("~/DieselSupervisor/Default.aspx");
                }
                else if (Convert.ToInt32(Request.Cookies["UserType"].Value) == 1)
                {
